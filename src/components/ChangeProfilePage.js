...
import { FormInput, FormCheckBox, FormSubmit, ConfirmRows } from './parts/form'
import { changeProfile, fetchProfile } from '../lib/card'
import { fetchAddressByPostal } from '../lib/api'

const { width, height, size } = Dimensions.get("window")

const isTablet = DeviceInfo.isTablet()

// クリックハンドラーなどが膨大なのでViewModel作って移譲させたい
// コンポーネントは本当はrenderだけに特化させたい
export default class ChangeProfilePage extends React.Component {
    state = {
    }
        ...
        return (
            <View>
                ...
                <View style={styles.formSectionLg}>
                    ...
                    <Text style={styles.notice}>データの反映には数日かかります。</Text>
                    <FormSubmit onSubmit={this.startConfirm} label="変更申請" />
                </View>
            </View>
        )
    }
    ...
    confirmContainer() {
        ...
        return (
            <View style={styles.confirmContainer}>
                ...
                <Text style={styles.confirmSentence}>こちらの内容に変更します。</Text>
                <Text style={styles.confirmSentence}>よろしければ確認ボタンを、前に戻る場合は</Text>
                <Text style={styles.confirmSentence}>戻るボタンをタップしてください。</Text>
                <View style={styles.confirmBtnGroup}>
                    ...
                    >
                        <Text style={styles.submitLabel}>確認</Text>
                    </TouchableOpacity>
                </View>
            </View>
        )
    ...
    UNSAFE_componentWillMount() {
        AsyncStorage.getItem('card_number').then(card_number => {
            if(!card_number) {
                return
            }
            fetchProfile(card_number).then(val => {

                // fixme: フォーム変更に伴いセットするkeyValを変更する必要あり。
                this.setState({
                    name: val.name,
                    postal: val.postal,
                    address: val.address,
    ...
    async onConfirm() {
        ...
        const params = {
            ...
            phone_number: `${this.state.phoneStart} ${this.state.phoneMiddle} ${this.state.phoneEnd}`,
            postal_code: `${this.state.postalStart} ${this.state.postalEnd}`,
        ...
        changeProfile(params).then(async (resp) => {
            Alert.alert("登録情報変更を受け付けました。")
            Actions.Card()
        }).catch(err => {
            errorHandle(err)
        })
